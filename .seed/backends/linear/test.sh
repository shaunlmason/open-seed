...
# (offline, CI-safe). Covers every required verb, the minted-token fence
# (incl. rotation), claim contention, the missing-workflow-state refusal,
# plan parking, the reviewer lockout, close refused outside review, and the
# cascade on close AND on cancel.
set -eu
...
say "OK: all required verbs; minted-token fence (incl. rotation), contention, parking, lockout, close+cancel cascades, close review-only, missing-state refusal all enforced"
