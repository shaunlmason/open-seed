...
func (e *Error) Error() string {
	return fmt.Sprintf("%s on %s refused: %s (next/spec/topology.md)", e.Verb, e.Subject, e.Reason)
}
...
func (e *NotReadyError) Error() string {
	...
	return fmt.Sprintf("claim.taken on %s refused: the contract is ready but not effectively ready, %s; it leaves the queue and the poll for the same reason and returns to both when that changes (next/spec/topology.md)", e.Subject, strings.Join(parts, " and "))
}
