...
// conformance: next/spec/curation.md "Search": an advisory lexical
...
func TestSectionDocumentsSplitOnHeadings(t *testing.T) {
	body := "preamble line\n\n# Title one\n\ntext one\n```\n## not a heading\n```\n## Sub two\n\ntext two\n## Sub two\n\ntext three\n### deeper stays inside\n"
	docs := SectionDocuments("next/docs/decisions.md", body)
	ids := make([]string, len(docs))
	for i, d := range docs {
		ids[i] = d.ID
	}
	want := []string{"next/docs/decisions.md", "next/docs/decisions.md#title-one", "next/docs/decisions.md#sub-two", "next/docs/decisions.md#sub-two-2"}
	...
	for p, ok := range map[string]bool{"next/docs/decisions.md": true, "a.md": true, "": false, "/etc/x": false, "../x": false, "a/../../x": false, ".": false, "..": false, "a//b": false, "./a": false} {
