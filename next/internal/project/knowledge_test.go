...
func TestKnowledgeProjectionPublishesTheStages(t *testing.T) {
	...
	if v := project.Report().Version; v != "17" {
		t.Fatalf("the report's version names the knowledge section (11, its retired and stale counts at 12), the lanes section after them (13), by_kind (15), the planner's strongest (16) and the adapters section (17): %s", v)
	}
