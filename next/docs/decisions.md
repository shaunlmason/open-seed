...
**The report version is "13", not the plan's "12".** Item 4 (os-0d537fbd)
took "12" for the `retired` and `stale` counts while this card was
planned, so the flywheel section moves the version once more; the
plan's figure named the next free number at planning time, and the
number is the derivation's, not the item's.
