...
implemented in #250 (in review: the hook's code-ref half authorizing
every non-ledger ref from the ledger and the `SEED_PUSHER` identity,
`internal/redteam` asserting the §I.2 ceiling both ways at the push,
and the protected surface restricted to the governance root); it names
its follow-up card os-32d06c65 for III.E row 8's reap arm. Items 2 through 6 are carded
