...
| 3 | `approval_refused` | an approval verb whose shape or citation is wrong ([`protocol.md`](protocol.md), "Per-verb approval"): a request naming no catalog verb, an approval verb, an unenrolled actor or a contract the chain does not know, a reason over one line or 200 bytes, an answer citing no request, another subject's request or an answered one, a grant with a reason, a denial without one |
