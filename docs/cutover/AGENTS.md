...
<!--
  STAGED, NOT LIVE. This is the root AGENTS.md as the self-hosting
  cutover will install it (docs/v1-retirement.md stage 3;
  next/docs/promotion.md, "The cutover and the rollback"). It is kept
  here rather than at the root so that merging the branch that wrote it
  is NOT the flip: authority moves when the cutover pull request moves
  this file into place, and not before.

  The cutover pull request does exactly two things with this file: `git
  mv docs/cutover/AGENTS.md AGENTS.md`, and deletes this comment.
-->
...
Everything below cites `next/spec/`, which is normative. Where this file
...
- **The declaration is deployment state**, at `seed.json`, read by the
  ...
  posture per `decisions/0005-cooperative-posture.md`: every writer
...
- **Append durable insights** to `memory/LEARNINGS.md` and failed
  approaches to `memory/DEADENDS.md` in your task pull request.
...
| `next/spec/` | The protocol: normative, and the authority over this file |
| `next/lanes/` | Role fragments: the grants and conventions each lane runs under |
| `next/bin/seed` | The binary every role file names |
...
verifiable from genesis by any reader (`seed ledger verify`). v1's
`seed-state` ref is frozen at its final anchor and kept read-only for its
history; `docs/v1-retirement.md` records what became of the rest.
