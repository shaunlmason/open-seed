...
**The planning phase and its mutex, precisely** (leases only cover live agents, and plan review routinely outlasts a lease): the planner claims the card (`ready → in_progress`, leased) and authors the plan; **on opening the plan PR, the shim parks the card `in_progress → blocked`** (blocked-on: plan approval) — a claim-ending act like any other exit from `in_progress`, and `blocked` cards are claim-free, unreaped, and *unclaimable*, so no rival planner can start while review is pending. When the plan PR merges, the maintenance workflow unblocks the card (`blocked → ready`, now carrying an approved plan at the default head); if the plan PR is closed unmerged, maintenance likewise unblocks it for re-planning. So the mutex is the lease *while authoring* and the blocked state *while in review*; a planner that crashes before opening the plan PR is reaped normally and another agent plans — which is the desired outcome, not a failure.
...
- **seed-maintenance** (live): **deterministic scripting only — requires no model secrets**: lease reaping (§7.1), orphaned-claim recovery (contrabass pattern: `in_progress` card with no live branch → release), **closing merged tasks** (merged `seed/<id>` PR + green verify ⇒ `seed task close <id>`, §7.1), **unblocking cards whose plan PR merged or closed** (§D1 planning phase), the **done-consistency lint** (every `done` card must correspond to a merged `seed/<id>` PR and a `receipts/<id>.json` on the default branch, **or** carry an explicit human no-PR resolution — `seed task close --no-pr <reason>` under a human operator credential records it, covering research/no-code cards; anything else is a forged accept and writes the HALT marker, §7.2), state-ref checkpoint anchoring (§7.2), contention-rate reporting, expiry cleanup of `by:agent` artifacts. Scheduled with scattered minutes (never `:00`), assuming missed ticks (aeon's debt-model catch-up). Mirror export joins this workflow in v2.
...
- **`close` is not a second path to done, and it happens after merge:** `close` = accept (`transition review → done`) plus the blocker-cascade, valid only from `review`; any other state is an invalid transition (exit 3). The normative sequence is **server PR approval → merge → close** — the cascade fires only once the default branch actually contains the work, so dependents never build on unmerged changes. The v1 caller is deterministic: a seed-maintenance step detects a merged `seed/<task-id>` PR with a green verify check and invokes `seed task close <task-id>` under the workflow's operator credential (a human lead may also close manually — using `--no-pr <reason>` for cards whose work never lands as a `seed/<id>` PR, so the done-consistency lint reads it as legitimate rather than forged). `cancel` is terminal from any non-terminal state and also cascades. The D1 transition table is the single authority; [research/10 Part 5](./research/10-org-control-planes.md) is amended by erratum accordingly (exit 6, `--token` on worker verbs, `close` restriction, `ready --squad` capability).
